...
if [[ ${#checks[@]} -eq 0 ]]; then
  ...
  if ! detected_raw="$(gh api "repos/$repo/commits/$head_sha/check-runs" --jq '.check_runs[].name')"; then
    echo "Failed to retrieve check runs for commit $head_sha from $repo" >&2
    exit 1
  fi
